...
class MetaSampleEvaluator(OperatorABC):
    ...
    def get_score(self, samples, input_key):
        ...
        for i, response in enumerate(responses):
            try:
                # reasoning 模型会在 <think> 里思考，分数列表在 <answer> 或 thinking 之后
                clean = extract_llm_answer_text(response or "")
                if not clean:
                    clean = response or ""
                # 找最后一个形如 [num, num, ...] 的行
                candidates = [ln.strip() for ln in clean.split("\n") if ln.strip()]
                parsed_scores = None
                for line in reversed(candidates):
                    if re.match(r"^\[[\d.,\s]+\]$", line):
                        try:
                            parsed_scores = ast.literal_eval(line)
                            break
                        except Exception:
                            continue
                if parsed_scores is None:
                    # fallback: 直接尝试最后一行
                    parsed_scores = ast.literal_eval(candidates[-1]) if candidates else None
