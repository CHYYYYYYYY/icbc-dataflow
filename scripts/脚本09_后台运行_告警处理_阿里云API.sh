#!/usr/bin/env bash
# 后台运行 TechDoc QA Pipeline V2 — 01-01 告警处理数据集（阿里云 qwen3.6-plus）
set -euo pipefail
...
export DF_MAX_WORKERS="${DF_MAX_WORKERS:-10}"
export NUM_QUESTIONS="${NUM_QUESTIONS:-4}"
export QA_RESUME="${QA_RESUME:-0}"
...
if [[ -f "$PID_FILE" ]]; then
  ...
  if kill -0 "$OLD_PID" 2>/dev/null; then
    ...
    echo "  进度: QA_OUTPUT_DIR=$QA_OUTPUT_DIR QA_FILE_PREFIX=$QA_FILE_PREFIX $SCRIPT_DIR/脚本02_查看QA管道运行进度.sh"
    echo "  日志: tail -f $LATEST_LOG_LINK"
    exit 1
  fi
fi

if [[ ! -f "$TECHDOC_INPUT" ]]; then
  echo "[ERROR] 输入不存在: $TECHDOC_INPUT"
  echo "请先运行 ocr_chunks_to_dataflow.py 转换 OCR JSON"
  exit 1
fi

echo "启动后台任务 — $DATASET_LABEL"
echo "  输入: $TECHDOC_INPUT"
echo "  前缀: $QA_FILE_PREFIX"
echo "  蒸馏标签: $DISTILL_TAG"
...
echo ""
echo "查看进度:"
echo "  QA_OUTPUT_DIR=\"$QA_OUTPUT_DIR\" QA_FILE_PREFIX=\"$QA_FILE_PREFIX\" $SCRIPT_DIR/脚本02_查看QA管道运行进度.sh --once"
echo ""
echo "查看日志:"
