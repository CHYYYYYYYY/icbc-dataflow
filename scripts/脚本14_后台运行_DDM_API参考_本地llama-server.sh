#!/usr/bin/env bash
# 后台运行 TechDoc QA Pipeline V2 — DDM API参考（本地 llama-server）
set -euo pipefail

SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
QA_OUTPUT_DIR="${QA_OUTPUT_DIR:-/home/wugk/finetune/finetune/data/分布式数据库中间件(DDM) 24.1.35.20 API参考(for 华为云Stack 8.3.1) 01}"
LOG_DIR="${LOG_DIR:-${QA_OUTPUT_DIR}/logs}"
mkdir -p "$LOG_DIR"

export QA_OUTPUT_DIR
export QA_CACHE_DIR="${QA_CACHE_DIR:-${QA_OUTPUT_DIR}/qa_pipeline_cache}"
export QA_FILE_PREFIX="${QA_FILE_PREFIX:-ddm_api_ref_qa}"
export TECHDOC_INPUT="${TECHDOC_INPUT:-${QA_OUTPUT_DIR}/text_chunks_20260402_151240_converted.json}"
export DATASET_LABEL="${DATASET_LABEL:-DDM API参考}"
export DISTILL_TAG="${DISTILL_TAG:-DDM}"
